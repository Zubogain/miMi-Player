...
import InputRangeComponent from '@Components/input'
import { useDispatch, useSelector } from 'react-redux'
import {
  asyncTrackSetVolume,
  asyncTrackToggle,
  asyncTrackVolumeMutedToggle,
} from '@Actions/track'
import { msToPrecent } from '@Utils/utils'
import { IRootState, AppDispatch } from '@Src/store'

const PlayerComponentSeeker = ({
  trackPrecent,
  deviceIsConnected,
}: {
  trackPrecent: number
  ...
  return (
    <div
      className={
        deviceIsConnected
          ? 'controls__seek-container relative disabled'
          : 'controls__seek-container relative'
      }
    >
      <InputRangeComponent
...
const PlayerComponentBluetoothControls = () => {
  return (
    <div className="footer relative">
      <div className="controls">
        <div className="controls-item timer disabled">--/--</div>
      </div>
...
const PlayerComponent = () => {
  ...
  const { bluetooth, track } = useSelector((state: IRootState) => ({
  ...
  return (
    <>
      ...
      {bluetooth.device.isConnected ? (
        ...
        <PlayerComponentControls
          isPlay={track.isPlay}
          duration={track.duration}
          position={track.position}
          volumeMuted={track.volumeMuted}
          volume={track.volume}
        />
      )}
    </>
